...
QUARANTINE="$HOME/.local/share/wolfcare_quarantine"
mkdir -p "$QUARANTINE"/{files,hashes,reports}
CONFIG_FILE="$QUARANTINE/.wolfcare_config"
MANIFEST="$QUARANTINE/hashes/quarantine_manifest.txt"
touch "$MANIFEST"
# User-maintained allowlist ("Mark as Safe") - sha256|label per line.
# Checked before every other detection layer, so a hash here is never
# re-flagged regardless of what ReleaseSeal/VT/MalwareBazaar/ClamAV/deep
# inspection say. For files a user has personally verified are safe -
# typically a PUP (bundled toolbar, adware installer, etc.) that isn't
# actually malicious, just unwanted - not a general-purpose override.
ALLOWLIST_FILE="$QUARANTINE/allowlist.txt"
...
check_dependencies() {
    ...
    if [ "${#missing[@]}" -gt 0 ]; then
        ...
        echo "    WolfCare cannot run without these. Install them and try again."
        exit 1
    fi
...
run_timeout() {
    ...
    local marker="${TMPDIR:-/tmp}/.wolfcare_rt_$$_$RANDOM"
    "$@" &
    local cmd_pid=$!
    (
        sleep "$secs"
...
banner() {
    ...
    echo ""
    echo "========================================"
    echo "   WolfCare Malware Scanner v1.1"
    echo "========================================"
}
...
manage_cron() {
    ...
    echo "========================================"
    echo "Current WolfCare cron jobs:"
    ...
    echo "  [3] Remove all WolfCare cron jobs"
    echo "  [4] Back"
    ...
    case "$cchoice" in
        ...
        3)
            ...
            echo -e "${GREEN}[v] WolfCare cron jobs removed${NC}"
            ;;
    esac
...
run_scan() {
    ...
    while IFS= read -r file; do
        ...
        # Checked before every other layer, including ClamAV/deep
        # inspection - a user-verified-safe hash skips all of it, not
        # just the hash-based checks.
        user_safe=$(check_user_allowlist "$sha")
        if [ -n "$user_safe" ]; then
            show_progress "$SCAN_COUNT" "$TOTAL_FILES"
            echo -e "\n${CYAN}[*] #$SCAN_COUNT: $name${NC}"
            echo -e "${GREEN}    [v] Marked safe by user: $user_safe${NC}"
            echo "  [v] USER-VERIFIED SAFE: $file | $sha | $user_safe" >> "$REPORT"
            ...
            [ "$GUI_MODE" -eq 1 ] && printf 'GUI\x1fFILE\x1f%s\x1fVERIFIED\x1f%s\x1f%s\x1f%s\x1f%s\x1f%s\n' \
                "$SCAN_COUNT" "$name" "$sha" "$file_size_kb" "Marked safe by user: $user_safe" "$file"
            continue
        fi
    ...
    if [ "$DETECTED" -gt 0 ] && [ -n "$ALERT_EMAIL" ]; then
        ...
        send_email_alert "WolfCare: $DETECTED threat(s) detected" \
            "WolfCare scanned $SCAN_COUNT files and quarantined $QUARANTINED.\nFull report: $REPORT"
    fi
