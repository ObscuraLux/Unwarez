    ...
    while IFS= read -r file; do
        ...
        if [ "$rs_status" = "COMPROMISED" ] || [[ "$vt_status" == MALICIOUS:* ]] || [[ "$vt_status" == PUP:* ]] || [ -n "$kb_match" ] || [[ "$mb_status" == MALICIOUS:* ]] || [[ "$clamav_status" == INFECTED:* ]] || [[ "$deep_status" == MALICIOUS:* ]]; then
            ...
            [ "$GUI_MODE" -eq 1 ] && printf 'GUI\x1fFILE\x1f%s\x1fVERIFIED\x1f%s\x1f%s\x1f%s\x1f%s\x1f%s\n' \
                "$SCAN_COUNT" "$name" "$sha" "$file_size_kb" "$rs_status/$vt_status/$circl_status${rs_cert:+ | Known release: $rs_cert}" "$file"
        else
            ...
            [ "$sig_status" != "N/A" ] && echo -e "${BLUE}    [i] Code signature: $sig_status${NC}"
            [ -n "$rs_cert" ] && echo -e "${BLUE}    [i] Known release certificate: $rs_cert (ReleaseSeal evidence only)${NC}"
            gui_unverified_detail="${sig_status/N\/A/no match}"
            if [[ "$deep_status" == SUSPICIOUS_SCRIPT:* ]]; then
                ...
                gui_unverified_detail="$gui_unverified_detail | Script warning: ${deep_status#SUSPICIOUS_SCRIPT:}"
            elif [[ "$deep_status" == RS_CERT:* ]]; then
                echo -e "${BLUE}    [i] Known release certificate: ${deep_status#RS_CERT:} (ReleaseSeal evidence only)${NC}"
                gui_unverified_detail="$gui_unverified_detail | Known release: ${deep_status#RS_CERT:}"
            fi
            [ -n "$rs_cert" ] && gui_unverified_detail="$gui_unverified_detail | Known release: $rs_cert"
            ...
            [ "$GUI_MODE" -eq 1 ] && printf 'GUI\x1fFILE\x1f%s\x1fUNVERIFIED\x1f%s\x1f%s\x1f%s\x1f%s\x1f%s\n' \
                "$SCAN_COUNT" "$name" "$sha" "$file_size_kb" "$gui_unverified_detail" "$file"
        fi
    done < "$FILE_LIST"
