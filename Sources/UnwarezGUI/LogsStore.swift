...
/// Reads the same per-scan report files the pipeline writes to
/// `UnwarezPaths.reportsDir` (one timestamped .txt per scan, listing
/// every file checked - not just detections) - so scans run from the
/// CLI show up here too, not just ones run from the GUI.
@MainActor
final class GUILogsStore: ObservableObject {
    @Published var entries: [LogEntry] = []

    var reportsDirPath: String { UnwarezPaths.reportsDir.path }
