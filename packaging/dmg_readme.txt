...
and Apple Silicon Macs. The same floor applies to the Terminal-only
unwarez-cli tool bundled inside the app (Contents/Resources/unwarez-cli)
and used for scheduled scans, since both share one native detection
engine now rather than a separate bash implementation with no version
floor of its own.
